...
const Statistics: React.FC<StatisticsProps> = ({ state }) => {
  ...
  const acceptancePercentage = ((state?.acceptance_ratio ?? 0) * 100).toFixed(1);
    ...
    <div className="bg-white rounded-lg shadow p-5 h-full flex flex-col space-y-8">
      ...
      <section className="flex flex-col space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-gray-50 px-4 py-3 rounded text-center">
            <p className="text-xs tracking-wide text-gray-600 mb-1"><InlineMath math={'\\hat{\\mu}_1'} /></p>
            <p className="text-lg font-semibold">{mu1Value.toFixed(3)}</p>
            <p className="text-[10px] text-gray-500">σ = {mu1StdDev.toFixed(3)}</p>
          </div>
          <div className="bg-gray-50 px-4 py-3 rounded text-center">
            <p className="text-xs tracking-wide text-gray-600 mb-1"><InlineMath math={'\\hat{\\mu}_2'} /></p>
            <p className="text-lg font-semibold">{mu2Value.toFixed(3)}</p>
            <p className="text-[10px] text-gray-500">σ = {mu2StdDev.toFixed(3)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-col space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div className="bg-gray-50 px-4 py-3 rounded text-center">
            <p className="text-xs uppercase tracking-wide text-gray-600 mb-1">Acceptance&nbsp;Rate</p>
